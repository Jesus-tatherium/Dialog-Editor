    ...
    public class DialogEditor : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (isVisible)
            {
                ...
                for (int i = xMinTextures; i <= xMaxTextures; i++)
                {
                    for (int j = yMinTextures; j <= yMaxTextures; j++)
                    {
        ...
        private void Inputs(Event e)
        {
            if (e.type == EventType.KeyDown)
            {
                ...
                {
                    KeyboardInput(e, KeyCode.Z, () => { dialogEditorCtrlZ.Backward(); GUI.FocusControl(null); e.Use(); });
                    KeyboardInput(e, KeyCode.Y, () => { dialogEditorCtrlZ.Forward(); GUI.FocusControl(null); e.Use(); });
        ...
        void isPointOnBezierCurve(Vector3 _pt)
        {
            //float p = (3 * ac - (b^2) / 3(a^2)
            //float q = (2(b^3) - 9abc + 27(a^2)d) / 27(a^3)
            //Result = startPos * Mathf.Pow(1 - t, 3) + 3 * controlPos1 * t * t * Mathf.Pow(1 - t, 2) + 3 * controlPos2 * t * t * (1 - t) + 3 * endPos;
        }
